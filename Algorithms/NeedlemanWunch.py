...
# In[6]:
#partally taken from https://www.biostars.org/p/231391/#231400 in their creation of the matrix using numpy 
...
def NeedlemanWunch(s1, s2, match, mismatch, gap):
    ...
    al_mat = numpy.zeros((m,n),dtype = int) #Initializes the alighment matrix with zeros.
    p_mat = numpy.zeros((m,n),dtype = str) #Initializes the alighment matrix with zeros.
    ...
    i = m-1
    j = n-1
    while( i > 0 and j > 0):#checking the path back 
        if(al_mat[i-1][j] > al_mat[i-1][j-1] and al_mat[i-1][j] > al_mat[i][j-1]):#editing the compared string based on the matrix run back 
            i = i - 1
            s2 = s2[:i+j] + '-' + s2[i+j:]
            
                
        elif(al_mat[i][j-1] > al_mat[i-1][j-1] and al_mat[i][j-1] > al_mat[i-1][j]):#editing the compared string based on the matrix run back 
            j = j - 1
            s2 = s2[:i+j] + '-' + s2[i+j:]
                
        elif(al_mat[i-1][j-1] > al_mat[i][j-1] and al_mat[i-1][j-1] > al_mat[i-1][j]):#does not need to be changed if going up diaganle 
            i = i - 1
            j = j - 1
